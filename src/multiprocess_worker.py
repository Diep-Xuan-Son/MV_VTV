...
from libs.utils import delete_folder_exist, logging, formatter
...
LOGGER = logging.getLogger("multiprocess_worker")
FILE_HANDLER = logging.FileHandler(f"{Config.PATH_LOG}{os.sep}multiprocess_worker_{datetime.now().strftime('%Y_%m_%d')}.log")
        ...
        while True:
            try:
                ...
                async for message in consumer:
                    ...
                    LOGGER.info(f"Consumer {consumer_id} received: {message.value} from partition {message.partition}")
                    ...
                    if res["success"]:
                        if res["result"]["status"] in ["deleted", "interrupted"] :
                            LOGGER.info(f"Message {data} has been deleted!")
                            continue
                    else:
                        LOGGER.error(f"The session ID {data['sess_id']} not found!")
                        continue
                ...
                LOGGER.warning(f"Consumer {consumer_id} is shutting down")
                await consumer.stop()
                break
            except Exception as e:
                tb_str = traceback.format_exc()
                LOGGER.error(f"Error in consumer {consumer_id}: {tb_str}")
            ...
            try:
                ...
                async for message in consumer:
                    ...
                    if res_mv["success"]:
                        ...
                        if res["success"]:
                            LOGGER.info("Upload final video success!")
